...
plt.rcParams.update({'font.size': 20}) # 14 # 18 # 22 is too big?
...
# otherwise check that we can insert a_predict_pow and calculate what to expect
    ...
    def plot_mag(self,show_powlist):
        ...
        for kk in show_powlist:
            if self.fit_flag[kk] == 1:
                ...
                plt.plot(self.f[kk],logmag(self.z[kk]),marker='o') # '.'
    ...
    def plot_phase(self,show_powlist):
        ...
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Phase (rad)') #r'$\theta$ (rad)') #'Residuals (rad)')

        plt.tight_layout()

        if self.save_fig:
            plt.savefig(self.filename + '_res' + str(self.res) + '_phase.png', format='png', dpi=600) # dpi=600
        elif self.save_pdf:
            pass
        else:
            plt.show()
    ...
    def plot_fit_results(self,a_predict_try=False,p_best=0,a_best=0):
        # make the fitted array for plotting purposes
        ...
        for kk in self.powlist:
            if self.fit_flag[kk] == 1:
                ...
                if a_predict_try:
                        axs[1, 1].plot(p_best,a_best,marker='x',color='red',linewidth=2) #markeredgecolor='red') # new, 6/12/24


        axs[0, 0].set_xlabel('Drive Power (dB)') # dBm
        axs[0, 0].set_ylabel('$f_{r}$ (Hz)')

        axs[0, 1].set_xlabel('Drive Power (dB)')
        axs[0, 1].set_ylabel('$Q_{i}$')

        axs[1, 0].set_xlabel('Drive Power (dB)')
        axs[1, 0].set_ylabel('$Q_{c}$')

        axs[1, 1].set_xlabel('Drive Power (dB)')
    ...
    def plot_fit_results_bif(self,a_predict_try=False,p_best=0,a_best=0): # new, 6/12/24
        # make the fitted array for plotting purposes
        ...
        for kk in self.powlist:
            if self.fit_flag[kk] == 1:
                ...
                print('a_predict', self.a_predict_guess, a_best) # self.a_predict
                # a vs microwave power
                plt.plot(kk,self.result[kk].result['bif'],'o')
        plt.axhline(self.a_predict_guess,color='gray',linestyle='--')
                #plt.axhline(self.a_predict)
                #plt.axvline(self.a_predict_pow2, color='gray')
                #plt.axvline(self.Pro_test_low_guess_dBm, color='red')
        if a_predict_try:
                plt.plot(p_best,a_best,marker='x',ms=20,markerfacecolor="None",markeredgecolor='red',markeredgewidth=5) #linewidth=2) #markeredgecolor='red') # new, 6/12/24 # ms=16
                #plt.axvline(p_best,color='red',linestyle='--')
        plt.grid()
        plt.xlabel('Drive Power (dB)')
        plt.ylabel('Nonlinearity Parameter a')

        plt.tight_layout()

        if self.save_fig:
            plt.savefig(self.filename + '_res' + str(self.res) + '_fit_results_bif.png', format='png') # dpi=600
        elif self.save_pdf:
            pass
        else:
            plt.show()
...
#        for ii in range(len(files)): 
    ...
    for ii in tone_range:
            ...
            if config['preview']['show_plots']:
                ...
                all_fits[ii].plot_mag(powlist)
                all_fits[ii].plot_phase(powlist)
                all_fits[ii].plot_pherr(powlist)
    ...
    # new, 6/11/24 # from dispatch_drivefit.py
    #a_ref=0.2, #DriveFitConfig.model_fields["a_ref"].default,
    #a_min=-0.2 #DriveFitConfig.model_fields["a_min"].default,
    #a_max=1.5, #DriveFitConfig.model_fields["a_max"].default,
    '''
...
if __name__ == "__main__":
    ...
    sweep_patterns = [
            ...
            f"toltec{nw}_{obsnum:06d}_*targsweep.nc"]
    ...
    kpf_ctx = do_toltec_drive_fit(config, nw, obsnum, sweep_files)
    chan_indices = kpf_ctx["tone_range"]
    all_fits = kpf_ctx["all_fits"]
    tone_freq_lo_all_0 = kpf_ctx['tone_freq_lo_all'][0]
    tone_amps_all_0 = kpf_ctx['tone_amps_all'][0]

    # extract fit info and interpolate to get best drivefit value
    drivefit_ctxs = {}
    for ci in chan_indices:
        fit_obj = all_fits[ci]
        drivefit_ctxs[ci] = find_best_adrv(fit_obj, a_ref=0.2) #cfg.a_ref) # self
    ...
    # skip as fits not as good as find_best_adrv
    '''
    drivefit_ctxsv2 = {}
    for ci in chan_indices:
        print(ci)
        fit_obj = all_fits[ci]
        drivefit_ctxsv2[ci] = find_best_adrv_extrapolate(fit_obj, a_ref=0.2) #cfg.a_ref) # self
    drivefit_flagsv2 = np.array([drivefit_ctxsv2[ci]["p_flag"] for ci in chan_indices])
    n_successv2 = np.sum(drivefit_flagsv2 == 0)
    resultv2 = locals()
    '''

    drivefit_p_best_all = []
    drivefit_a_best_all = []
    for ii in chan_indices: # same as config['tone_range']
        drivefit_p_best_all.append(result['drivefit_ctxs'][ii]['p_best'])
        drivefit_a_best_all.append(result['drivefit_ctxs'][ii]['a_best'])
    drivefit_p_best_all = np.asarray(drivefit_p_best_all)
    drivefit_a_best_all = np.asarray(drivefit_a_best_all)

    #plt.figure()
    #plt.scatter(drivefit_p_best_all,drivefit_a_best_all) # not so informative since drivefit_a_best_all is similar
    #plt.xlabel('Readout Power (dB)')
    #plt.ylabel('Nonlinearity parameter a')
    #plt.show()

    Pro_guess_dBm_list = []
    for ii in chan_indices:
        Pro_guess_dBm_list.append(all_fits[ii].Pro_test_low_guess_dBm)
    # saved Pro_guess_dBm_list_pos to csv file above
    Pro_guess_dBm_list_pos = np.asarray(Pro_guess_dBm_list)
    Pro_guess_dBm_list_neg_indices = np.where(Pro_guess_dBm_list_pos < 0)
    drivefit_p_best_all_test = drivefit_p_best_all[Pro_guess_dBm_list_neg_indices] 
    drivefit_p_best_all_test_2 = drivefit_p_best_all_test[drivefit_p_best_all_test != np.array(None)]
    #Pro_test[~np.isnan(Pro_test)] # not needed for now

    # for histogram(s)
    print(np.shape(drivefit_p_best_all_test_2))
    print(np.shape(Pro_guess_dBm_list_pos[Pro_guess_dBm_list_neg_indices]))
    #L1 = np.abs(np.max(drivefit_p_best_all[Pro_guess_dBm_list_neg_indices]) - np.min(drivefit_p_best_all[Pro_guess_dBm_list_neg_indices]))
    #L2 = np.abs(np.max(Pro_guess_dBm_list_pos[Pro_guess_dBm_list_neg_indices]) - np.min(Pro_guess_dBm_list_pos[Pro_guess_dBm_list_neg_indices]))
    #print(L1, L2)

    plt.figure()
    plt.hist(drivefit_p_best_all_test_2,color='blue') #,bins=np.round(L2/20))
    #plt.hist(Pro_guess_dBm_list_pos[Pro_guess_dBm_list_neg_indices],alpha=0.5,color='orange') #bins=np.round(L2/20),alpha=0.5)
    plt.axvline(x=np.median(drivefit_p_best_all_test_2),color='blue')
    plt.xlabel('Readout Power for reference a')
    plt.ylabel('Number of resonators')
    plt.xlim(-30,0) # based on drive attenuation range
    plt.tight_layout()
    plt.show()

    plt.figure()
    plt.plot(tone_freq_lo_all_0[Pro_guess_dBm_list_neg_indices],drivefit_p_best_all_test,marker='o',linestyle='None')
    #plt.plot(tone_freq_lo_all_0[Pro_guess_dBm_list_neg_indices],Pro_guess_dBm_list_pos[Pro_guess_dBm_list_neg_indices],marker='s',linestyle='None')
    plt.xlabel('Tone frequency (Hz)')
    plt.ylabel('Readout power identified (dB)')
    plt.tight_layout()
    plt.show()

    plt.figure()
    plt.plot(tone_freq_lo_all_0,tone_amps_all_0,marker='o',linestyle='None')
    plt.xlabel('Tone frequency (Hz)')
    plt.ylabel('Tone amplitude')
    plt.tight_layout()
    plt.show()

    # print(result) # variable (dict) has a lot of stored information
    if config['fit_settings']['tone_range'] == 'all': #tone_range:
        print('config[tone_range] is all, exiting the code')
        sys.exit()
    else:
        for ii in chan_indices: # similar to config['tone_range']
            print('saving figures for res' + str(ii))
            print(result['drivefit_ctxs'][ii]['p_best'])
            #ctx = all_fits[ii].plot()
            #plt.close(ctx)
            if result['drivefit_ctxs'][ii]['p_best']:
                all_fits[ii].plot_fit_results_bif(a_predict_try=True,p_best=result['drivefit_ctxs'][ii]['p_best'],a_best=result['drivefit_ctxs'][ii]['a_best'])
            else:
                pass
