'''
resonance_fitting_swv3.py # give better name
...
from numba import jit # new, 1/29/23
import argparse
...
class LoadVNAsweep(): # appears to work for both file types
    ...
    # does this work as is or do I need more self
...
class ResonanceFitterSingleTone(): # Analyzesingletone()
    ...
    def __init__(self,f,z,tau,numspan=2,tone_freq_lo=0,window_width=0,\
        ...
        #self.pherr_mean = pherr_mean
        self.flag_threshold = flag_threshold

        # new argument for specifying output to print to user
        self.verbose = verbose
        
        self.window_width = window_width # new, 1/22/23
        
        if ('weight_type' in keywords): # new, 1/23/23
    ...
    def windowlorentz(self,f): # new, 1/23/23
    ...
    def windowgauss(self,f): # new, 1/23/23
        # f_c is the resonant frequency or probe tone frequency + LO
        f_c = self.tone_freq_lo
        x = 2. / self.window_width * (f - f_c) # 2/W * (f - f_c)
    ...
    def fits21sim(self,showplot=False,use_bounds=False):
        ...
        #r, zc, c_residue = circlefittest(z1[id1:id2])
        circle_obj = Circlefit(z1[id1:id2]) # Circlefit(z1[id1:id2])
        result['r'] = float(circle_obj.r) #float(r)
        result['zc'] = circle_obj.zc #zc
        result['c_residue'] = circle_obj.residue #c_residue
        ...
        # can we combine these into the line above?
        result['popt'] = popt
        result['Q'] = popt[0]
        result['f0'] = popt[1]
        result['phi'] = popt[2]
        ...
        result['fit_result'] = fit_result # with trimmed data
        ...
        angt_to_iq = result['A_magt']*(np.cos(result['fit_result']-np.angle(-result['ezinf'])) + 1.j*np.sin(result['fit_result']-np.angle(-result['ezinf'])))
    ...
    def fphasenlinsave(self, f, Q, alpha, f00, phi, r, z): # f is the independent variable here
        # self, f, Q, alpha, f00, phi, r, z
        A = z - r*np.exp(1.j*(phi+np.pi))
        f0 = f00 - alpha*np.abs(A)**2
        zoff = r*np.exp(1.j*(phi+np.pi))
        x = (f - f0)/f00 #/f0
    
        ang = phi + 2.*np.arctan(-2.*Q*x)
        return ang, x, A, f0, zoff # 2/9/23, now saving A # 2/13/23, now saving f0 # A and f0 are arrays in matlab too
    ...
    def fitphasenlintest2(self, ft, angt, Q, alpha, f0, phi, r, zt, **keywords):
        ...
        if ('bounds' in keywords):
            ...
            if ('err' in keywords):
                ...
                try:
                    ...
                    fit_result, xt, At, f0t, zoff = self.fphasenlinsave(ft,popt[0],popt[1],popt[2],popt[3],r,zt)
                except RuntimeError:
        ...
        return popt, pcov, fit_result, p0_result, xt, uncertainty_angt, At, f0t, zoff
    ...
    def fitphasenlin_c2(self,f,z,estv,numspan=2,use_bounds=False):
        ...
        if use_bounds:
            ...
            popt, pcov, fit_result, x0_result, xt, uncertainty_angt, At, f0t, zoff = self.fitphasenlintest2(ft, angt, Qgg, alphag, f00g, phig, estv['r'], zt, 
                                                                bounds = bounds)
        else:
            popt, pcov, fit_result, x0_result, xt, uncertainty_angt, At, f0t, zoff = self.fitphasenlintest2(ft, angt, Qgg, alphag, f00g, phig, estv['r'], zt)
        ...
        return popt, pcov, pherr, fit_result, x0_result, ft, zt, angt, ang, var, err, xt, flag, uncertainty_angt, ezinf, At, f0t, zoff
    ...
    def fits21simnlin(self,use_bounds=False,showplot=False):
        ...
        # now fit to data below
        if use_bounds:
            popt, pcov, pherr, fit_result, x0_result, ft, zt, angt, ang, var, err, xt, flag, uncertainty_angt, ezinf, At, f0t, zoff = self.fitphasenlin_c2(self.f,z2,estv,numspan,use_bounds=use_bounds)
        else:
            popt, pcov, pherr, fit_result, x0_result, ft, zt, angt, ang, var, err, xt, flag, uncertainty_angt, ezinf, At, f0t, zoff = self.fitphasenlin_c2(self.f,z2,estv,numspan)
        ...
        # more correct?, actually the same as zd_err, keep zd_err and zinf_err
        #result['zd_err2'] = np.sqrt((-result['zc']/np.abs(result['zc'])*1.j*np.exp(1.j*result['phi'])*2.*result['r'])**2*result['popt_err'][3]**2)
        result['zinf_err'] = np.sqrt((result['zc']/np.abs(result['zc'])*1.j*np.exp(1.j*result['phi'])*result['r'])**2*result['popt_err'][3]**2)

        #projection of zf0 into zinf
        l = np.real(result['zf0']*np.conj(result['zinf']))/np.abs(result['zinf'])**2
        result['l'] = l
        #result['l2'] = np.real(result['zf0']/result['zinf']) # same as l, has been checked
    ...
    def plot(self):
        ...
        plt.subplot(224)
        plt.plot(self.result['ft'],self.result['pherr']) # self.angle -self.fitted_angle
        plt.xlabel('Frequency (Hz)')
...
class ResonanceFitterSingleTonePowSweep():
    ...
    def __init__(self,filedir,data,res,tone_freq_lo,powlist_full,powlist,\
                 ...
                 filename='toltec',**keywords): # data is new, 12/12/22
        # f in Hz (will also work with GHz?), z is complex, tau in ns
        '''
            data needs to be complex (like I + jQ)
        '''
        ...
        if ('result0' in keywords):
            ...
            if (np.abs(np.mean(z0_mag_test)-np.amin(z0_mag_test)) < 2e4): # make user variable
                ...
                self.powlist = self.powlist[1:] #self.powlist_new
        ...
        for kk in fit_pows: #range(0,len(self.powlist)):
            ...
            # make fits easier to troubleshoot by saving partial guesses etc.
            ...
            if self.id2[kk_p] < len(self.f[kk_p]): # this is needed I think
            ...
            self.f0gid[kk_p] = find_nearest_indice(self.f[kk_p],self.f0g[kk_p]) # did we define this somewhere already?

        # don't calculate E_star with negative bif values
        ...
        print('pos bif', np.asarray(self.bif_list[1:])[self.bif_flag_filter]) # this appears to work
        if np.asarray(self.bif_list[1:])[self.flag_filter].size == 0:
             ...
             self.bif_list_pos = np.abs(self.bif_list) # not using for now
             ...
             self.a_predict_off = self.Pro_test_low_dBm/self.a_predict_pow2
             ...
             if np.abs(self.a_predict_off-1.) > self.a_predict_threshold: #2.*self.flag_threshold: #2.*self.flag_threshold:
                 self.a_predict_flag = 1
             else:
...
# otherwise check that we can insert a_predict_pow and calculate what to expect
    ...
    def plot(self):
        ...
        for kk in self.powlist:
            if self.fit_flag[kk] == 1:
                ...
                axs[1, 0].plot(self.f[kk],self.result[kk].result['ang'],'.') #,color='r')

                axs[1, 0].plot(self.result[kk].result['ft'],self.result[kk].result['angt'],'o',color=color) # c
    ...
    def plot_phase(self,show_powlist):
        ...
        for kk in show_powlist:
            if self.fit_flag[kk] == 1:
                ...
                plt.plot(self.f[kk],self.result[kk].result['ang'],'.') #,color='r')
                color = plt.gca().lines[-1].get_color() # get most recent line color
    ...
    def plot_fit_results(self): # come up with a better name
        # make the fitted array for plotting purposes
        ...
        for kk in self.powlist:
            if self.fit_flag[kk] == 1:
                ...
                #axs[0, 0].plot(kk,self.result[kk].result['f0'],'*',color=color)
...
class ToltecLOsweep():
    ...
    def create_data_structure(self):
        ...
        #print('testing, f array is', np.shape(f_array_hz))
...
class Pvna():
    ...
    def __init__(self,tone_data):
        ...
        #self.f2,self.y2 = self.make_pseudo_vna_sweep2()
...
if __name__ == "__main__":
        ...
        parser.add_argument('-a', '--a_predict_guess')
        parser.add_argument('-at', '--a_predict_threshold')
        parser.add_argument('-pt', '--pherr_threshold_num')
        parser.add_argument('-num', '--numspan')
        parser.add_argument('-res', '--res_list',nargs='+', type=int, help='<Required> Set flag')
        args = parser.parse_args()
        print(args.filename)
        print(args.a_predict_guess)
        print(args.res_list)
	
        # for toltec data
        sweep_dir_1 = '/home/sam/Downloads/autodrive_data_17067_17068_toltec0/'
        save_dir = '/home/sam/Downloads/autodrive_data_17067_17068_toltec0/figures_test/'
        if args.filename != None:
            sweep_name_1 = str(args.filename)
        else:
            sweep_name_1 = 'toltec0_017068'
        print(sweep_name_1)
        split_sweep = sweep_name_1.split('_')
        network_num = int(''.join(filter(str.isdigit, split_sweep[0])))
        print(split_sweep)
        print(network_num) # this works better
        
        use_save_fig = False #True #True
        use_save_pdf = False
        use_save_file = False
        ...
        window_Qr = 12000 # user specified value # new, 1/23/23

        start = timer()
        
        files = glob.glob(sweep_dir_1+sweep_name_1+'_*') # sweep_dir_2+sweep_name_2
        files.sort()
        #files.reverse()
        #print(files)
        ...
        # same tone_freq_lo_all for all files # same size as num_tones
        window_width = tone_freq_lo_all[0]/window_Qr
        ...
        if args.a_predict_guess != None:
            use_a_predict_guess = float(args.a_predict_guess)
        else:
            use_a_predict_guess = 0.2

        if args.a_predict_threshold != None:
            use_a_predict_threshold = float(args.a_predict_threshold)
        else:
           use_a_predict_threshold = 0.25 #0.2 #0.15 # > than this value, lower is more stringent
           
        if args.pherr_threshold_num != None:
            use_pherr_threshold_num = int(args.pherr_threshold_num)
        else:
           use_pherr_threshold_num = 5 # 10 by default, new 2/1/23 # doesn't seem to change things much
           
        if args.numspan != None:
            use_numspan = args.numspan
        else:
            use_numspan = 1 #2 #3 # 1
        print(use_a_predict_guess, use_a_predict_threshold, use_pherr_threshold_num, use_numspan)
        ...
        if args.res_list != None:
            tone_range = args.res_list
        else:
            tone_range = range(0,num_tones) #[0] #[301] #[10] #[301] #[10] #range(0,num_tones) #[0] #[10] #range(0,num_tones)
        ...
        for ii in tone_range:
                ...
                all_fits[ii] = ResonanceFitterSingleTonePowSweep(0,data_pow_sweep,ii,tone_freq_lo_all[0][ii],powlist_full,powlist,\
                                                                 ...
                                                                 flag_threshold=use_a_predict_threshold,pherr_threshold_num=use_pherr_threshold_num,\
                ...
                all_fits[ii].plot_iq_rect([-15]) # testing this out
                
                # appears we want to avoid using the negative bif values
        ...
        if use_save_pdf:
            ...
            with PdfPages('fits_skyv1-1_' + sweep_name_1 + '_test.pdf') as pdf:
                for ii in tone_range:
                    ...
                    ctx = all_fits[ii].plot()
                    pdf.savefig(ctx,dpi=100) # seems to be ignoring this?
                    plt.close(ctx)
        ...
        #Pro_guess_dBm_good = Pro_guess_dBm_good[Pro_guess_dBm_good < 0]
        Pro_guess_dBm_good = Pro_guess_dBm_good[Pro_guess_dBm_good < -5.] # this is temporary until I'm able to improve the fits
        ...
        #print(np.bincount(np.asarray(Pro_guess_dBm_list)[a_predict_flag_good_indices]))
        ...
        indices_mean_sorted = np.argsort(f0_corr_all_from_mean) # will want to skip f0_corr_all with 0 # np.sort to sort the lists this way # can then decide what resonators to skip
        ...
        if use_save_file:
            with open('fits_skyv1-1_' + sweep_name_1 + '_lorentz_weight.pkl', 'wb') as handle: #fits_labv1.pkl
                pickle.dump(a_save, handle) #, protocol=pickle.HIGHEST_PROTOCOL)
        
        '''
        tune_drive_atten = -14 # or something else?
        f0_corr_all = []
        a_all = []
        f0_corr_all_2 = []
        a_all_2 = []
        tone_num = []
        for ii in a_predict_flag_good_indices[0]: #range(0,num_tones):
	        f0_corr_all.append(all_fits_2[ii][tune_drive_atten]['f0_corr'])
	        a_all.append(all_fits_2[ii][tune_drive_atten]['bif'])
	
        for ii in tone_range:
            if len(all_fits_2[ii]) == 0:
                a_all_2.append(np.inf)
                f0_corr_all_2.append(np.inf)
                tone_num.append(ii)
            else:
                a_all_2.append(all_fits_2[ii][tune_drive_atten]['bif'])
                f0_corr_all_2.append(all_fits_2[ii][tune_drive_atten]['f0_corr'])
                tone_num.append(ii)
        a_all_2 = np.asarray(a_all_2)
        a_all = np.asarray(a_all)
        f0_corr_all = np.asarray(f0_corr_all)
        f0_corr_all_2 = np.asarray(f0_corr_all_2)

        plt.figure()
        plt.plot(f0_corr_all_2,a_all_2,marker='o',linestyle='None',label='all resonators')
        plt.plot(f0_corr_all,a_all,marker='o',linestyle='None',label='good resonators flagged')
        plt.xlim((4.e8,10.e8))
        plt.ylim((-2.1,2.1))
        plt.xlabel('Resonator f0')
        plt.ylabel('Nonlinearity parameter a')
        plt.legend()
        plt.show()
        '''
